...
void GeometryTextFormatter::consume(const ObjectPosition& f) {
    ...
    if (f.fix_level >= FixLevel::kStaleFix) {
        sprintf(databuf, "%d,%d,%d\0\0", (int)((f.pos[0]+2)*1000), (int)((f.pos[1]+2)*1000), (int)((f.pos[2]+2)*1000));
        printer.printf("%s", databuf);
        // printer.printf("%d,%d,%d", (int)((f.pos[0]+2)*1000), (int)((f.pos[1]+2)*1000), (int)((f.pos[2]+2)*1000));
